...
def main() -> None:
    ...
    for time_frame in ["All Time", "Solar Cycle 24"]:
        ...
        for index, flare_class in enumerate(["B", "N", "M", "X"]):
            new_df = all_flares_df.loc[
                (all_flares_df["xray_class"] == flare_class)
            ]
            if time_frame == "Solar Cycle 24":
                new_df = new_df.loc[
                    (new_df["time_start"].str.contains("2013")) |
                    (new_df["time_start"].str.contains("2014"))
                ]
            counts_df.at[index, "total"] = new_df.shape[0]
            for occurrence in ["singular", "multiple"]:
                occurrence_df = new_df.loc[
                    (new_df["xray_class"] == flare_class) &
                    (new_df["ARs"] == occurrence)
                ]
                counts_df.at[index, occurrence] = occurrence_df.shape[0]
                for coincidence in COINCIDENCES:
                    df = occurrence_df
                    if coincidence == "coincident":
                        df = occurrence_df.loc[occurrence_df["COINCIDENCE"] == True]
                    elif coincidence == "noncoincident":
                        df = occurrence_df.loc[occurrence_df["COINCIDENCE"] == False]
                    counts_df.at[index, f"{occurrence}_{coincidence}"] = df.shape[0]
