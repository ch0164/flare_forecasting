...
def time_series_vector_classification3():
    ...
    # param_list = [param for param in powerset(FLARE_PROPERTIES) if (len(param) == 1)]
    param_list = ["TOTUSJZ", "TOTUSJH", "SAVNCPP"]
    # print(param_list)
    # exit(1)
    ...
    df = pd.concat([
        pd.read_csv(f"{other_directory}TOTUSJZ.csv"),
        pd.read_csv(f"{other_directory}TOTUSJH.csv"),
        pd.read_csv(f"{other_directory}SAVNCPP.csv"),
    ], axis=1)
    params_len_2 = [params for params in powerset(list(set(FLARE_PROPERTIES) - set(param_list))) if len(params) == 2]
    print(params_len_2)
    for param in params_len_2:
        param1, param2 = param
        params = list(param) + param_list
        df = pd.concat([df, pd.read_csv(f"{other_directory}{param1}.csv")], axis=1)
        df = pd.concat([df, pd.read_csv(f"{other_directory}{param2}.csv")],
                       axis=1)
        # Counts: {'x': 25, 'm': 434, 'c': 140, 'b': 784, 'n': 355}
        # if len(params) == 1:
        #     param1 = params[0]
        ...
        for coincidence, coin_value in zip(["all"], [None]):
            # mean_tss = {name: [] for name in names}
            series_tss = {name: [] for name in names}
        ...
        results_df.to_csv(f"{metrics_directory}classification/selected_quintuples_totusjh_totusjz_savncpp.csv", index=False)
...
def classification_plot2():
    ...
    pair_df = pd.read_csv(f"{metrics_directory}classification/all_params_pairs.csv")
    triple_df = pd.read_csv(f"{metrics_directory}classification/all_params_triples.csv")

    for time_series in pair_df["label"].unique():
        plot_df = pd.DataFrame()
        df = pair_df.sort_values(by="tss", ascending=False)
        df = df.loc[df["label"] == time_series]
        best_params = df.head(5)["parameter"].tolist()
        print(time_series, best_params)

        for name in names:
            name_df = df.loc[df["name"] == name]
            name_df = name_df.loc[name_df["parameter"].isin(best_params)].head(5)
            plot_df = pd.concat([plot_df, name_df])
            # exit(1)

        # def combine_label(row):
        #         return " ".join(row.values.astype(str))
        # plot_df["full_label"] = plot_df[["parameter", "label"]].apply(combine_label, axis=1)

        ax = sns.barplot(data=plot_df, x="name", y="tss", hue="parameter")
        plt.legend(bbox_to_anchor=(1.05, 1.0), loc='upper left')
        ax.set_title(f"NBC vs. MX Classification Comparison, {time_series}")
        ax.set_ylim(bottom=0.0, top=1.0)
        ax.set_ylabel("TSS")
        ax.set_xlabel("Classifier")
        plt.tight_layout()
        plt.savefig(
            f"{figure_directory}pairs_and_triples/top_five_pairs_on_{time_series}.png")
        plt.show()
        plt.clf()


def plot_parameter_tss():
    df = pd.read_csv(f"{metrics_directory}classification/all_params_triples.csv", index_col=False)
    df = df.sort_values(by="tss", ascending=False)
    sns.set_theme(style="whitegrid")
    for time_series in df["label"].unique():
        for name in ["SVM", "RFC", "KNN"]:
            temp_df = df.loc[df["label"] == time_series]
            temp_df = temp_df.loc[temp_df["name"] == name]
            temp_df = temp_df.head(20)
            f, ax = plt.subplots(figsize=(6, 8))
            sns.set_color_codes("pastel")
            sns.barplot(x="tss", y="parameter", data=temp_df,
                        label="TSS", color="y")
            ax.legend(ncol=1, loc="lower right", frameon=True)
            ax.set(xlim=(0, 1), ylabel="",
                   xlabel="TSS", title=f"{time_series}, {name} TSS, Top 20 Triples")
            sns.despine(left=True, bottom=True)
            plt.tight_layout()
            f.savefig(f"{figure_directory}top_params/{time_series}_{name}_triples.png")
            plt.show()
            plt.clf()
...
def generate_parallel_coordinates(coincidence, all_flares_df):
    ...
    parallel_coordinates(normalized_df, "xray_class", FLARE_PROPERTIES, ax, sort_labels=flare_classes,
